...
const MAIN_CATEGORIES = [
  { id: 'prompt-engineering', name: '提示词工程', nameEn: 'Prompt Engineering', icon: '🧠', count: 12, color: '#FF6B35' },
  { id: 'ai-tools', name: '智能工具', nameEn: 'AI Tools', icon: '💻', count: 84, color: '#3B82F6' },
  { id: 'writing', name: '写作创作', nameEn: 'Writing', icon: '✍️', count: 8, color: '#8B5CF6' },
  { id: 'image-gen', name: '图像生成', nameEn: 'Image Generation', icon: '🎨', count: 6, color: '#EC4899' },
  { id: 'data-analysis', name: '数据分析', nameEn: 'Data Analysis', icon: '📊', count: 5, color: '#10B981' },
  { id: 'business', name: '商业营销', nameEn: 'Business', icon: '💼', count: 7, color: '#F59E0B' },
  { id: 'translate', name: '翻译语言', nameEn: 'Translation', icon: '🌐', count: 4, color: '#06B6D4' },
];
...
export default function SystemPromptsPage() {
  ...
  const handleMainCategoryClick = (id: string) => { setSelectedMainCategory(id); setCurrentView('category'); setSelectedSubCategory('all'); setSelectedPrompt(null); };
  ...
  const renderMainMenu = () => (
    <div style={{ backgroundColor: COLORS.bg }} className="min-h-screen">
      ...
      <main className="px-6 py-12 max-w-7xl mx-auto">
        <div className="text-center mb-16"><h2 style={{ fontFamily: '-apple-system, SF Pro Display, Microsoft YaHei, sans-serif', color: COLORS.text }} className="text-5xl font-bold mb-4">MYSTERY SPELL BOX</h2><p style={{ color: COLORS.textSecondary }} className="text-lg">探索 AI 工具思维方式的秘密世界</p></div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {MAIN_CATEGORIES.map((cat, i) => (
            <button key={cat.id} onClick={() => handleMainCategoryClick(cat.id)} onMouseEnter={() => setHoveredCard(i)} onMouseLeave={() => setHoveredCard(null)} style={{ backgroundColor: COLORS.bgSecondary, borderColor: hoveredCard === i ? cat.color : COLORS.border }} className={`relative p-8 rounded-xl text-left transition-all duration-500 hover:shadow-xl hover:-translate-y-2 border ${hoveredCard === i ? 'scale-105' : 'scale-100'}`}>
              ...
              <h3 style={{ fontFamily: '-apple-system, SF Pro Display, Microsoft YaHei, sans-serif', color: COLORS.text }} className="text-xl font-bold mb-2">{cat.name}</h3>
  ...
  const renderCategoryPage = () => (
    <div style={{ backgroundColor: COLORS.bg, minHeight: '100vh', color: COLORS.text }} className="flex flex-col">
      <header style={{ backgroundColor: COLORS.bgSecondary, borderBottom: `1px solid ${COLORS.border}` }} className="h-16 flex items-center justify-between px-6 sticky top-0 z-50">
        <div className="flex items-center gap-4">
          <button onClick={selectedSubCategory !== 'all' ? handleBackToCategory : handleBackToMenu} style={{ color: COLORS.textSecondary }} className="flex items-center gap-2 hover:text-white"><ArrowLeft className="w-5 h-5" /></button>
          <div style={{ backgroundColor: COLORS.border }} className="w-px h-6" />
        ...
        <div style={{ backgroundColor: COLORS.bgSecondary, borderBottom: `1px solid ${COLORS.border}` }} className="relative py-4">
          <div className="flex items-center gap-2 px-4 max-w-7xl mx-auto">
            ...
            <div ref={categoryListRef} className="flex gap-2 overflow-x-auto scrollbar-hide flex-1" style={{ scrollbarWidth: 'none' }}>
              <button onClick={() => { setSelectedSubCategory('all'); setCategorySlideIndex(0); }} style={{ backgroundColor: selectedSubCategory === 'all' ? COLORS.accent : COLORS.bgTertiary, color: selectedSubCategory === 'all' ? '#000' : COLORS.textSecondary }} className="px-4 py-1.5 text-sm font-medium rounded-full whitespace-nowrap hover:scale-105">全部</button>
              {allCategories.map(cat => <button key={cat} onClick={() => handleCategoryClick(cat)} style={{ backgroundColor: selectedSubCategory === cat ? COLORS.accent : COLORS.bgTertiary, color: selectedSubCategory === cat ? '#000' : COLORS.textSecondary }} className="px-4 py-1.5 text-sm font-medium rounded-full whitespace-nowrap hover:scale-105">{cat}</button>)}
            </div>
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6">
      {loading ? (
        ...
        <pre style={{ backgroundColor: COLORS.bgSecondary, border: `1px solid ${COLORS.border}`, color: COLORS.textSecondary }} className="whitespace-pre-wrap font-mono text-sm p-6 rounded-xl overflow-x-auto">{content}</pre>
      )}
    </div>
  );
}
